...
# DSpark adaptive verification (vLLM enable_adaptive_verification) is NOT offered: this
# image's DeepseekV4IndexerBackend refuses it at KV init ("Adaptive verification trims
# verification requests on device, which the DeepseekV4IndexerBackend attention backend
# does not support"), after ~9 min of weight loading (2026-09-24 trial,
# docs/spec-adaptive-and-nccl-heartbeat-20260924.md).
...
_dsv41_validate_spec_and_heartbeat() {
    if [[ " ${EXTRA_ARGS:-} " =~ enable_adaptive_verification ]]; then
        echo "enable_adaptive_verification: this image's DeepseekV4IndexerBackend refuses it at KV init (after ~9 min of loading) — see docs/spec-adaptive-and-nccl-heartbeat-20260924.md" >&2
        return 2
    fi
